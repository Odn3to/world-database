...
echo "$($PSQL "SELECT AVG(total_goals) AS average_goals FROM (SELECT winner_goals + opponent_goals AS total_goals FROM games) as subquery")"
...
echo "$($PSQL "SELECT name FROM games left join teams ON games.winner_id = teams.team_id where round = 'Eighth-Final' and year = 2014 union SELECT name FROM games left join teams ON games.opponent_id = teams.team_id where round = 'Eighth-Final' and year = 2014")"
